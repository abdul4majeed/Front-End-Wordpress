    ...
    <div class="box">
        ...
        <div class="box-body" id="refreshme">
            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                @foreach ($teams_header as $team_header)
                 <tr class="stu{{$team_header->id}}">
                    <td>
                       <p>{{$team_header->title}}</p>
                    </td>
                    <td>
                    ...
                    <td>
                        <form action="{{route('teamHeader.edit',[$team_header->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        {{-- <form action="{{route('teamHeader.destroy',[$team_header->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
