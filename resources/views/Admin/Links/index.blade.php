            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($links as $link)
                 <tr class="stu{{$link->id}}">
                    ...
                    <td>
                        {{-- <form action="{{route('link.edit',[$link->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        <form action="{{route('link.destroy',[$link->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
