            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($abouts_us as $about_us)
                 <tr class="stu{{$about_us->id}}">
                    <td>
                       <p>{{$about_us->head_title}}</p>
                    </td>
                    <td>
                        <p>{{$about_us->head_description}}</p>
                    </td>
                  
                    <td>
                        <form action="{{route('aboutUsExperienceProcess.edit',[$about_us->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        {{-- <form action="{{route('aboutUsExperienceProcess.destroy',[$about_us->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
