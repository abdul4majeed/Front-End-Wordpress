            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($abouts_us as $about_us)
                 <tr class="stu{{$about_us->id}}">
                    ...
                    <td>
                    <p><img src="{{asset('about/experience/image')}}/{{$about_us->image_url}}" alt="{{$about_us->title}}" ></p>
                    </td>
                  
                    <td>
                        <form action="{{route('aboutUsBodyExperienceSection.edit',[$about_us->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        {{-- <form action="{{route('aboutUsBodyExperienceSection.destroy',[$about_us->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
