    ...
    <div class="box">
        ...
        <div class="box-body" id="refreshme">
            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                @foreach ($testimonials_header as $testimonial_header)
                 <tr class="stu{{$testimonial_header->id}}">
                    <td>
                       <p>{{$testimonial_header->title}}</p>
                    </td>
                    <td>
                        <p>{{$testimonial_header->description}}</p>
                    </td>
                  
                    <td>
                        <form action="{{route('testimonialHeaderSection.edit',[$testimonial_header->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        {{-- <form action="{{route('testimonialHeaderSection.destroy',[$testimonial_header->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
