            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($link_contents as $link_content)
                 <tr class="stu{{$link_content->id}}">
                    <td>
                       <p>{{$link_content->link['link']}}</p>
                    </td>
                    <td>
                        
                    <p>{{substr($link_content->content,0,20)}}><b>{{"Click Update Button To Update Content"}}</b>
                    </td>
                  
                    <td>
                        <form action="{{route('link_content.edit',[$link_content->id])}}" method="get" class="form-inline-style">
                            @csrf
                            <button type="submit" class="btn-xs btn-success btn-style"><i class="fa fa-edit"></i>Update</button>
                        </form>
                        <form action="{{route('link_content.destroy',[$link_content->id])}}" method="post" class="form-inline-style">
                            @csrf
                            @method('delete')
